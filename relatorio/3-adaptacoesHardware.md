...
Na segunda imagem, a árvore do "Netlist Navigator" (painel da esquerda) está expandida até dentro do `fp_adder:fp_add_unit`, e dá pra ver uma peça isolada do 4º estágio: um mux rotulado `expn~[7:4]`. Essa é exatamente a lógica de decisão do expoente final do `fp_adder.vhd`, a que escolhe entre carry-out, underflow ou caso normal, já sintetizada como hardware de verdade: um multiplexador selecionando entre as três opções calculadas em paralelo.
